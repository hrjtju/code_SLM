...
class QNet(nn.Module):
    ...
    def __init__(self, 
        ...
        self.input_2 = nn.Sequential(
            nn.Linear(in_features=self.V+self.M+self.P - max_part, out_features=256),
            nn.LeakyReLU(inplace=True),
        )
    ...
    def forward(self, x: Tensor) -> Tuple[Tensor, Tensor]:
        ...
        a = x[:, self.V + self.M :: 3 + 4 * self.max_ori]
        
        all_cols = torch.arange(x.shape[1], device=self.device)
        mask = ~torch.isin(all_cols, all_cols[self.V + self.M :: 3 + 4 * self.max_ori])
        b = x[:, mask]
...
class DoubleDQN:
    def __init__(self, 
        ...
        self.target_q_net.apply(init_weights_normal)
